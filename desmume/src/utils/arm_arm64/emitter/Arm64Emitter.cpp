...
#define ASSERT_MSG(location, condition, message, ...)                          \
  if (!(condition)) {                                                          \
...
bool FPImm8FromFloat(float value, u8 *imm_out) {
  ...
  const u8 imm8 = (sign << 7) | ((!(exponent >> 7)) << 6) |
...
void ARM64XEmitter::EncodeCompareBranchInst(u32 op, ARM64Reg Rt,
  ...
  ASSERT_MSG(DYNA_REC, !(distance & 0x3),
...
void ARM64XEmitter::EncodeExceptionInst(u32 instenc, u32 imm) {
  ASSERT_MSG(DYNA_REC, !(imm & ~0xFFFF),
...
void ARM64XEmitter::EncodeCondCompareImmInst(u32 op, ARM64Reg Rn, u32 imm,
  ...
  ASSERT_MSG(DYNA_REC, !(imm & ~0x1F), "%s: too large immediate: %d", __func__,
             imm);
  ASSERT_MSG(DYNA_REC, !(nzcv & ~0xF), "%s: Flags out of range: %d", __func__,
             nzcv);
...
void ARM64XEmitter::EncodeLoadRegisterInst(u32 bitop, ARM64Reg Rt, u32 imm) {
  ...
  ASSERT_MSG(DYNA_REC, !(imm & 0xFFFFF), "%s: offset too large %d", __func__,
             imm);
...
void ARM64XEmitter::EncodeLoadStoreIndexedInst(u32 op, u32 op2, ARM64Reg Rt,
  ...
  ASSERT_MSG(DYNA_REC, !(imm < -256 || imm > 255), "%s: offset too large %d",
             __func__, imm);
...
void ARM64XEmitter::EncodeLoadStoreIndexedInst(u32 op, ARM64Reg Rt, ARM64Reg Rn,
  ...
  ASSERT_MSG(DYNA_REC, imm >= 0,
             "%s(INDEX_UNSIGNED): offset must be positive %d", __func__, imm);
  ASSERT_MSG(DYNA_REC, !(imm & ~0xFFF),
             "%s(INDEX_UNSIGNED): offset too large %d", __func__, imm);
...
void ARM64XEmitter::EncodeMOVWideInst(u32 op, ARM64Reg Rd, u32 imm,
  ...
  ASSERT_MSG(DYNA_REC, !(imm & ~0xFFFF), "%s: immediate out of range: %d",
             __func__, imm);
...
void ARM64XEmitter::EncodeAddSubImmInst(u32 op, bool flags, u32 shift, u32 imm,
  ...
  ASSERT_MSG(DYNA_REC, !(imm & ~0xFFF), "%s: immediate too large: %x", __func__,
             imm);
...
void ARM64XEmitter::EncodeLoadStoreUnscaled(u32 size, u32 op, ARM64Reg Rt,
  ...
  ASSERT_MSG(DYNA_REC, !(imm < -256 || imm > 255),
             "%s received too large offset: %d", __func__, imm);
  Rt = DecodeReg(Rt);
  Rn = DecodeReg(Rn);
...
void ARM64XEmitter::QuickCallFunction(ARM64Reg scratchreg, const void *func) {
  ...
  if (!IsInRangeImm26(distance)) {
    // WARN_LOG(DYNA_REC, "Distance too far in function call (%p to %p)! Using
    // scratch.", m_code, func);
...
static void GetSystemReg(PStateField field, int &o0, int &op1, int &CRn,
  ...
  default:
    ASSERT_MSG(DYNA_REC, false,
               "Invalid PStateField to do a register move from/to");
    break;
  }
}
...
void ARM64XEmitter::SXTW(ARM64Reg Rd, ARM64Reg Rn) {
  ASSERT_MSG(DYNA_REC, Is64Bit(Rd), "%s requires 64bit register as destination",
             __func__);
  SBFM(Rd, Rn, 0, 31);
}
...
void ARM64XEmitter::LDURSW(ARM64Reg Rt, ARM64Reg Rn, s32 imm) {
  ASSERT_MSG(DYNA_REC, !Is64Bit(Rt),
             "%s must have a 64bit destination register!", __func__);
...
void ARM64XEmitter::ABI_PushRegisters(BitSet32 registers) {
  ...
  ASSERT_MSG(DYNA_REC, it == registers.end(), "%s registers don't match.",
             __func__);
}
...
void ARM64FloatEmitter::EmitScalar2Source(bool M, bool S, u32 type, u32 opcode,
                                          ...
                                          ARM64Reg Rm) {
  ASSERT_MSG(DYNA_REC, !IsQuad(Rd),
             "%s only supports double and single registers!", __func__);
  Rd = DecodeReg(Rd);
  Rn = DecodeReg(Rn);
  Rm = DecodeReg(Rm);
...
void ARM64FloatEmitter::EncodeLoadStorePair(u32 size, bool load, IndexType type,
  ...
  if (size == 128) {
    ASSERT_MSG(DYNA_REC, !(imm & 0xF), "%s received invalid offset 0x%x!",
               __func__, imm);
    opc = 2;
    imm >>= 4;
  } else if (size == 64) {
    ASSERT_MSG(DYNA_REC, !(imm & 0x7), "%s received invalid offset 0x%x!",
               __func__, imm);
    opc = 1;
    imm >>= 3;
  } else if (size == 32) {
    ASSERT_MSG(DYNA_REC, !(imm & 0x3), "%s received invalid offset 0x%x!",
               __func__, imm);
    opc = 0;
    imm >>= 2;
  }
...
void ARM64FloatEmitter::LD1(u8 size, u8 count, ARM64Reg Rt, ARM64Reg Rn) {
  ASSERT_MSG(DYNA_REC, !(count == 0 || count > 4),
...
void ARM64FloatEmitter::LD1(u8 size, u8 count, IndexType type, ARM64Reg Rt,
  ...
  ASSERT_MSG(DYNA_REC, !(count == 0 || count > 4),
             "%s must have a count of 1 to 4 registers!", __func__);
  ASSERT_MSG(DYNA_REC, type == INDEX_POST, "%s only supports post indexing!",
             __func__);
...
void ARM64FloatEmitter::FMOV(ARM64Reg Rd, ARM64Reg Rn, bool top) {
  if (IsScalar(Rd) && IsScalar(Rn)) {
    ...
    ASSERT_MSG(DYNA_REC, !IsQuad(Rd) && !IsQuad(Rn),
               "FMOV can't move to/from quads");
    int rmode = 0;
    int opcode = 6;
    int sf = 0;
...
void ARM64FloatEmitter::SMOV(u8 size, ARM64Reg Rd, ARM64Reg Rn, u8 index) {
  bool b64Bit = Is64Bit(Rd);
  ASSERT_MSG(DYNA_REC, Rd < SP, "%s destination must be a GPR!", __func__);
  ASSERT_MSG(DYNA_REC, size != 64,
             "%s doesn't support 64bit destination. Use UMOV!", __func__);
  u32 imm5 = 0;
...
void ARM64FloatEmitter::SSHLL(u8 src_size, ARM64Reg Rd, ARM64Reg Rn, u32 shift,
                              bool upper) {
  ASSERT_MSG(DYNA_REC, shift < src_size,
...
void ARM64FloatEmitter::SHRN(u8 dest_size, ARM64Reg Rd, ARM64Reg Rn, u32 shift,
                             bool upper) {
  ASSERT_MSG(DYNA_REC, shift < dest_size,
             "%s shift amount must less than the element size!", __func__);
  u32 immh = 0;
  u32 immb = shift & 0xFFF;
...
void ARM64FloatEmitter::FMUL(u8 size, ARM64Reg Rd, ARM64Reg Rn, ARM64Reg Rm,
                             u8 index) {
  ASSERT_MSG(DYNA_REC, size == 32 || size == 64,
             "%s only supports 32bit or 64bit size!", __func__);
...
void ARM64FloatEmitter::MOVI(u8 size, ARM64Reg Rd, u64 imm, u8 shift) {
  ...
  if (size == 8) {
    ASSERT_MSG(DYNA_REC, shift == 0, "%s(size8) doesn't support shift!",
               __func__);
    ASSERT_MSG(DYNA_REC, !(imm & ~0xFFULL),
               "%s(size8) only supports 8bit values!", __func__);
  } else if (size == 16) {
    ASSERT_MSG(DYNA_REC, shift == 0 || shift == 8,
               "%s(size16) only supports shift of {0, 8}!", __func__);
    ASSERT_MSG(DYNA_REC, !(imm & ~0xFFULL),
               "%s(size16) only supports 8bit values!", __func__);
    ...
    ASSERT_MSG(DYNA_REC, !(imm & ~0xFFULL),
               "%s(size32) only supports 8bit values!", __func__);
    switch (shift) {
    case 8:
      cmode |= 2;
      break;
  ...
  {
    ASSERT_MSG(DYNA_REC, shift == 0, "%s(size64) doesn't support shift!",
               __func__);
    ...
    for (int i = 0; i < 8; ++i) {
      u8 tmp = (imm >> (i << 3)) & 0xFF;
      ASSERT_MSG(DYNA_REC, tmp == 0xFF || tmp == 0,
                 "%s(size64) Invalid immediate!", __func__);
      if (tmp == 0xFF)
        abcdefgh |= (1 << i);
    }
  }
...
void ARM64FloatEmitter::MOVI2F(ARM64Reg Rd, float value, ARM64Reg scratch,
                               bool negate) {
  ASSERT_MSG(DYNA_REC, !IsDouble(Rd),
             "MOVI2F does not yet support double precision");
  uint8_t imm8;
  if (value == 0.0) {
